...
{
    // Lista backlogs do projeto (útil na aba do projeto)
    public function index(Project $project)
    {
        $backlogs = Backlog::where('project_id', $project->id)
            ->latest()
            ->paginate(10);
    ...
    public function store(Request $r, Project $project)
    {
        $data = $r->validate([
            ...
            'type'        => 'required|in:feature,bugfix,new screen',
    ...
    {
        // proteção extra: garante que o backlog pertence ao project
        if ($backlog->project_id !== $project->id) {
            abort(404);
        }

        return view('backlogs.edit', [
            'project' => $project,
            'backlog' => $backlog,
            'users'   => User::orderBy('name')->get(),
        ]);
